...
fn main() -> Result<(), impl Error> {
    ...
    let pipeline = {
        ...
        let layout = PipelineLayout::new(
            device.clone(),
            // Since w only have one pipeline in this example, and thus one pipeline layout,
            // we automeatically generate the creation info for it from the resources used in the
